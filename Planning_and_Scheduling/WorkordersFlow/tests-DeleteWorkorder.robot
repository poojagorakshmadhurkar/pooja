...
#    Wait For Elements State    //span[text()="${itemData1}[0]"]/../../../../../../../../../td[2][text()="${processname}"]    visible   timeout=30s
    ...
    #wait until element is visible   //span[text()="${itemData1}[0]"]/../../../../../../../../../td[2][text()="${processname}"]/../td[5][text()="${itemData1}[2]"]
...
#    Wait For Elements State        //div[text()="${work_order_number}"]/../../td[4][text()="${machine}[0]"]/../td[5]//span[text()="${itemData1}[0]"]/../../../../../../../../../td[6][text()="${processname}"]   visible   timeout=30s
    #scheduling page
    Switch Page        ${all_windows[0]}
    Log    Switched to window: ${all_windows[0]}
    # Delete Workorder
    Workorder buttons  7  ${work_order_number}
    click  ${reviewbutton}
    click  ${deactivateworkorderbutton}
    Reload
    #validation if workorder deletion
    ${updated_workorder_ids} =    Get All Workorders IDs
    Log    Updated Work Order IDs: ${updated_workorder_ids}  # Log to check its value

# Check if no work orders are found and log it as success (case passed)
    Run Keyword If    '${updated_workorder_ids}' == ''    Log    No work orders found after deletion.    AND    RETURN

    ${updated_workorder_ids} =    Create List    ${updated_workorder_ids}  # Wrap it in a list
    ${is_deleted} =    Evaluate    '${newly_workorder_id}' not in ${updated_workorder_ids}

# Log success if the work order is deleted, otherwise fail the test
    Run Keyword If    ${is_deleted}    Log    Work order ${newly_workorder_id} has been successfully deleted.
...    ELSE    Fail    Work order ${newly_workorder_id} still exists after deletion!
