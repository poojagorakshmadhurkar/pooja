...
*** Variables ***
@{customername}  Test01
@{itemData1}  FG-item2  1000  50  100
...
#    Wait For Elements State    //span[text()="${itemData1}[0]"]/../../../../../../../../../td[2][text()="${processname}"]     visible   timeout=30s
    #workorder edition
    click  ${workorderEditbutton}
    Input  (//input[@id='label'])[1]  100
    ${total_time1} =    Calculate Total Time    ${time_string}    ${itemData1}[3]
    Log    Total time for 100 units: ${total_time}
    click  ${reviewbutton}
    click  ${EDITWORKORDER}
    I Should See Text On Page        Work Order Edited Successfully
    click  //div[@data-task-id="${newly_workorder_id}"]
    Wait For Elements State    //div[text()="${customername}[0]"]/../../..//a[text()="${order_number}"]    visible   timeout=30s
    Wait For Elements State    (//div[text()="${machine}[0]"])[2]/../../../../div[2]//div[text()="${total_time1}"]    visible   timeout=30s
#    Wait For Elements State    //span[text()="${itemData1}[0]"]/../../../../../../../../../td[2][text()="${processname}"]    visible   timeout=30s
#    wait until element is visible    //span[text()="${itemData1}[0]"]/../../../../../../../../../td[6]//div[text()="${itemData1}[3]"]
...
#    Wait For Elements State       //span[text()="${itemData1}[0]"]/../../../../../../../../../td[2][text()="${processname}"]     visible   timeout=30s
    Wait For Elements State       //div[text()="${total_time1}"]/../../../../..//div[text()="${machine}[0]"]/../..//div[text()="${work_order_number}"]     visible   timeout=30s
